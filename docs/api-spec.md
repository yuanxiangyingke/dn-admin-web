...
- **说明**：返回有序的菜单树。根据角色编码标记已分配的权限。

**Query 参数**

| 参数 | 类型 | 说明 |
| ---- | ---- | ---- |
| `roleCode` | string | 角色编码，必填。响应中的 `assigned` 字段会标记该角色拥有的权限节点 |
...
### 5.5 菜单树节点结构 <a id="menu-tree-node"></a>
...
| `assigned` | boolean | 当传入 `roleId` 时标记该角色是否拥有此节点关联的权限 |
